// Task1. Задайте двумерный массив. Напишите программу, которая упорядочит по убыванию элементы каждой строки двумерного массива.
// Например, задан массив:
// 1 4 7 2
// 5 9 2 3
// 8 4 2 4
// В итоге получается вот такой массив:
// 7 4 2 1
// 9 5 3 2
// 8 4 4 2

// int[,] Create2DRandomArray(int rows, int cols, int minValue, int maxValue)
// {
//     int[,] array = new int[rows, cols];

//     for (int i = 0; i < rows; i++)
//     {
//         for (int j = 0; j < cols; j++)
//         {
//             array[i, j] = new Random().Next(minValue, maxValue);
//         }
//     }
//     return array;
// }

// void Print2DArray(int[,] array)
// {
//     for (int i = 0; i < array.GetLength(0); i++)
//     {
//        for (int j = 0; j < array.GetLength(1); j++)
//        {
//            Console.Write(array[i, j] + " ");
//        }
//        Console.WriteLine();
//     }
//     Console.WriteLine();
// }

// int[,] SortedRows(int[,] array)
// {
//     for (int i = 0; i < array.GetLength(0); i++)
//     {
//         for (int j = 1; j < array.GetLength(1); j++)
//         {
//             for (int n = 0; n < array.GetLength(1) - 1; n++)
//             {
//                 if (array[i, n] > array[i, n + 1])
//                 {
//                     int temp = array[i, n];
//                     array[i, n] = array[i, n + 1];
//                     array[i, n + 1] = temp;
//                 }
//             }
//         }
//     }
//     return array;
// }

// Console.Write("Enter the number of rows: ");
// int rows = Convert.ToInt32(Console.ReadLine());
// Console.Write("Enter the number of columns: ");
// int columns = Convert.ToInt32(Console.ReadLine());
// Console.Write("Enter the minimum value: ");
// int minValue = Convert.ToInt32(Console.ReadLine());
// Console.Write("Enter the maximum value: ");
// int maxValue = Convert.ToInt32(Console.ReadLine());

// int[,] myArray = Create2DRandomArray(rows, columns, minValue, maxValue);

// Print2DArray(myArray);

// Print2DArray(SortedRows(myArray));



// Task2. Задайте прямоугольный двумерный массив. Напишите программу, которая будет находить строку с наименьшей суммой элементов.
// Например, задан массив:
// 1 4 7 2
// 5 9 2 3
// 8 4 2 4
// 5 2 6 7
// Программа считает сумму элементов в каждой строке и выдаёт номер строки с наименьшей суммой элементов: 1 строка


// int SumLine(int[,] array, int i)
// {
//     int sumLine = array[i, 0];

//     for (int j = 1; j < array.GetLength(1); j++)
//     {
//         sumLine += array[i, j];
//     }
//     return sumLine;
// } 

// int MinSumLine(int[,] array)
// {
//     int minSumLine = 0;
//     int sumLine = SumLine(array, 0);

//     for (int i = 0; i < array.GetLength(0); i++)
//     {
//         int temp = SumLine(array, i);
//         if (sumLine > temp) minSumLine = i;
//     }
//     return minSumLine + 1;
// }

// void SumLineElement(int[,] array)
// {
//     for (int i = 0; i < array.GetLength(0); i++)
//     {
//         int temp = SumLine(array, i);
//         Console.WriteLine(temp);
//     }
// }    

// Print2DArray(myArray);
// Console.WriteLine(MinSumLine(myArray));
// Console.WriteLine(" ");
// SumLineElement(myArray);



// Task3. Задайте две матрицы. Напишите программу, которая будет находить произведение двух матриц.
// Например, даны 2 матрицы:
// 2 4 | 3 4
// 3 2 | 3 3
// Результирующая матрица будет:
// 18 20
// 15 18


// int[,] MultiplyMatrix(int[,] firstMatrix, int[,] secondMatrix, int rowsFirstMatrix, int colsCecondMatrix)
// {
//     int[,] resultMatrix = new int[rowsFirstMatrix, colsCecondMatrix];

//     for (int i = 0; i < resultMatrix.GetLength(0); i++)
//     { 
//         for (int j = 0; j < resultMatrix.GetLength(1); j++)
//         {
//             int sum = 0;
//             for (int n = 0; n < firstMatrix.GetLength(1); n++)
//             {
//                 sum += firstMatrix[i, n] * secondMatrix[n, j];
//             }
//             resultMatrix[i,j] = sum;
//         }
//     }
//     return resultMatrix;
// }

// Console.Write("Enter the number of rows first matrix: ");
// int rowsFirstMatrix = Convert.ToInt32(Console.ReadLine());
// Console.Write("Enter the number of cols first matrix (rows second matrix): ");
// int colsFirstMatrix = Convert.ToInt32(Console.ReadLine());
// Console.Write("Enter the number of cols second matrix: ");
// int colsCecondMatrix = Convert.ToInt32(Console.ReadLine());

// int rowsSecondMatrix = colsFirstMatrix;
// int minValue = 0;
// int maxValue = 2;

// Console.WriteLine("First Matrix:");
// int[,] firstMatrix = Create2DRandomArray(rowsFirstMatrix, rowsSecondMatrix, minValue, maxValue);
// Print2DArray(firstMatrix);

// Console.WriteLine("Second Matrix:");
// int[,] secondMatrix = Create2DRandomArray(colsFirstMatrix, colsCecondMatrix, minValue, maxValue);
// Print2DArray(secondMatrix);

// Console.WriteLine("Multiply Matrix:");
// Print2DArray(MultiplyMatrix(firstMatrix, secondMatrix, rowsFirstMatrix, colsCecondMatrix));



// Task4. Сформируйте трёхмерный массив из неповторяющихся двузначных чисел. Напишите программу, которая будет построчно выводить массив, добавляя индексы каждого элемента.
// Массив размером 2 x 2 x 2
// 66(0,0,0) 25(0,1,0)
// 34(1,0,0) 41(1,1,0)
// 27(0,0,1) 90(0,1,1)
// 26(1,0,1) 55(1,1,1)

// int[,,] Create3DRandomArray(int x, int y,int z)
// {
//     int[,,] array = new int[x, y, z];

//     int[] temp = new int[array.GetLength(0) * array.GetLength(1) * array.GetLength(2)];
//     int number;
//     for (int i = 0; i < temp.GetLength(0); i++)
//     {
//         temp[i] = new Random().Next(10, 100);
//         number = temp[i];
//         if (i >= 1)
//         {
//             for (int j = 0; j < i; j++)
//             {
//                  while (temp[i] == temp[j])
//                 {
//                     temp[i] = new Random().Next(10, 100);
//                     j = 0;
//                     number = temp[i];
//                 }
//                 number = temp[i];
//              }
//          }
//     }
//     int count = 0; 
//     for (x = 0; x < array.GetLength(0); x++)
//     {
//         for (y = 0; y < array.GetLength(1); y++)
//         {
//             for (z = 0; z < array.GetLength(2); z++)
//             {
//                 array[x, y, z] = temp[count];
//                 count++;
//             }
//         }
//     } 
//     return array;
// }

// void Print3DArray(int[,,] array)
// {
//     Console.WriteLine();
//     for (int i = 0; i < array.GetLength(0); i++)
//     {
//        for (int j = 0; j < array.GetLength(1); j++)
//        {
//            for (int k = 0; k < array.GetLength(2); k++)
//            {
//                 Console.Write( $"{array[i,j,k]}({i} {j} {k}) ");
//            }
//            Console.WriteLine();
//        }
//     }
//     Console.WriteLine();
// }

// Console.Write("Enter X: ");
// int x = Convert.ToInt32(Console.ReadLine());
// Console.Write("Enter Y: ");
// int y = Convert.ToInt32(Console.ReadLine());
// Console.Write("Enter Z: ");
// int z = Convert.ToInt32(Console.ReadLine());

// Print3DArray(Create3DRandomArray(x, y, z));



// Task5. Напишите программу, которая заполнит спирально массив 4 на 4.
// Например, на выходе получается вот такой массив:
// 01 02 03 04
// 12 13 14 05
// 11 16 15 06
// 10 09 08 07

// int[,] SpiralArray(int[,] array)
// {
//     int temp = 1;
//     int i = 0;
//     int j = 0;

//     while (temp <= array.GetLength(0) * array.GetLength(1))
//     {
//         array[i, j] = temp;
//         temp++;
//         if (i <= j + 1 && i + j < array.GetLength(1) - 1) j++;
//         else if (i < j && i + j >= array.GetLength(0) - 1) i++;
//         else if (i >= j && i + j > array.GetLength(1) - 1) j--;
//         else i--;
//     }
//     return array;
// }

// void PrintArray(int[,] array)
// {
//     Console.WriteLine();
//     for (int i = 0; i < array.GetLength(0); i++)
//     {
//        for (int j = 0; j < array.GetLength(1); j++)
//        {
//            if (array[i, j] / 10 == 0) Console.Write($"0{array[i, j]} ");
//            else Console.Write(array[i, j] + " ");
//        }
//        Console.WriteLine();
//     }
//     Console.WriteLine();
// }


// int size = 4;
// int[,] myArray = new int[size, size];

// PrintArray(SpiralArray(myArray));